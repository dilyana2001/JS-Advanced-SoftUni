...
const editMovieTemplate = (onSubmit) => html `
<div class="edit-movie-container">
  <form @submit=${onSubmit}>
              ...
              <div class="mb-3 row" style="width: 200px";>
                      <input type="submit" value="Edit Movie" class="form-control" />
              </div>
   </form>
</div>`;
...
export function editMoviePage(context) {
    const onSubmit = (e) => {
        ...
        movieService.editMovie(context.params.id, title, imageUrl, description)
            .then(() => {
                alert(`Movie changed!`)
    ...
    context.render(editMovieTemplate(onSubmit));
}